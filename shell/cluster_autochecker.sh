...
r_cmd='ssh -p 26387 -o StrictHostKeyChecking=no'
...
chk_diskused(){
  ...
  do
  {
    cat $TOOL_LOG/dfinfo | grep $dir &> /dev/null || echo "DiskLose:$dir"
    $r_cmd $ip "touch $dir/hadoop/aaa && rm -f $dir/hadoop/aaa &> /dev/null || echo -e \"DiskWrite:$dir\"" 
  } &
  done
  wait
...
chk_proc(){
  ip=$1;proc=$2
  $r_cmd $ip ps ax |grep "$proc" &> /dev/null || echo "$proc is down"
}
